...
**What it does:** Lets Hermes see your screen to understand context. When enabled, Hermes can detect which app you're in and adjust formatting accordingly.

**Without it:** Hermes will still work, but it may not detect context as accurately. You'll get good formatting, but it might not be app-specific.

<Note>
  **Privacy note:** Hermes only takes a screenshot when you dictate, uses it to detect context, and immediately discards it. Screenshots are never stored or sent anywhere.
</Note>
...
<Steps>
  ...
  <Step title="Navigate to Screen Recording">
    Go to **Privacy & Security → Screen Recording** (or **Screen & System Audio Recording** on newer macOS versions)
  </Step>
  <Step title="Enable Hermes">
    Find Hermes and toggle it **on**
  </Step>
...
| Screen Recording | No | Context detection |
